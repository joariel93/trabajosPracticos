...
printf("[1] Cargar los datos de los empleados desde el archivo data1.csv (modo texto)\n");
...
printf("[8] Guardar los datos de los empleados en el archivo data1.csv (modo texto)\n");
printf("[9] Guardar los datos de los empleados en el archivo data2.bin (modo binario)\n");
printf("[10] \n");
...
int utn_getStringInt (char *pResultado,char *pMensaje,char *pMensajeError,int minimo,int maximo,int limite)
{
	...
	int retorno = -1;
	char text[limite];
	if(	pResultado != NULL && pMensaje	!= NULL &&	0< limite)
	{
		while(flagError==-1)
		{
			flagError=0;
